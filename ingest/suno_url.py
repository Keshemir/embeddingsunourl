...
def _try_metadata_tags(html: str) -> str | None:
    """Extract Suno's `metadata.tags` blob from the page's serialized state.

    On share pages this is a free-text style description authored either by
    the user or auto-generated by Suno's chirp-fenix model — e.g.
    "Russian pop-rock track featuring a male vocalist with a slightly raspy,
    emotive delivery. The arrangement is built around a bright acoustic
    guitar..." It's the closest thing to ground-truth genre/style we have
    and is far more reliable than zero-shot CLAP/MuQ tagging on a one-author
    Suno corpus.

    The string is JSON-encoded inside the HTML, so we unescape `\\n`, `\\"`, etc.
    """
    # Suno share pages have the JSON state escape-encoded inside an HTML
    # attribute or <script> string, so quotes appear as `\"` and the
    # `metadata.tags` value lives as:
    #     \"metadata\":{\"tags\":\"Russian pop-rock track...\"
    # We match that escaped form first, then fall back to plain JSON.
    patterns = (
        # 1. Escaped JSON (the common case on share pages)
        r'\\"metadata\\"\s*:\s*\{[^}]*?\\"tags\\"\s*:\s*\\"(.*?)\\"',
        # 2. Plain JSON (in case Suno serves it un-escaped on some pages)
        r'"metadata"\s*:\s*\{[^}]*?"tags"\s*:\s*"((?:[^"\\]|\\.){10,4000})"',
        # 3. Last resort: any `tags` field with a long-enough value
        r'\\"tags\\"\s*:\s*\\"([^\\]{30,4000}?)\\"',
        r'"tags"\s*:\s*"((?:[^"\\]|\\.){30,4000})"',
    )
    for pat in patterns:
        ...
        if not m:
            continue
        raw = m.group(1)
        # Unescape: ANY backslash-escapes that survived the page render
        try:
            import json as _json
            # Wrap into a JSON string literal, normalizing the doubly-escaped form
            normalized = raw.replace('\\\\"', '\\"')   # `\\"` → `\"` (already escaped)
            return _json.loads(f'"{normalized}"')
        except Exception:
            return (raw.replace('\\"', '"')
                       .replace('\\n', '\n')
                       .replace('\\\\', '\\'))
...
def fetch_meta(url: str) -> SunoMeta:
    ...
    # Always try the inline metadata.tags blob — it's much richer than the
    # `tags=...` field above (which is usually empty on share pages) and
    # far more useful as a search signal than og:description.
    if blob := _try_metadata_tags(html):
        # If we had nothing useful, take it as both prompt and tags. If we
        # already have a `tags` (from JSON-LD or __NEXT_DATA__), keep it but
        # also store the blob into prompt so search has the descriptive text.
        if not tags or len(blob) > len(tags) * 2:
            tags = blob
        if not prompt or prompt.lower().startswith("listen and make"):
            prompt = blob

    # Description fallback from meta tags — only if we still have nothing
    if not prompt:
